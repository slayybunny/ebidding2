            ...
            <div class="row justify-content-center">
                <div class="col-md-8 text-center mb-4">
                    @php
                        ...
                        $avatarPath = public_path('storage/avatars/admin/' . $avatarFile);
                        $avatarUrl =
                            !empty($avatarFile) && file_exists($avatarPath)
                                ? asset('storage/avatars/admin/' . $avatarFile)
                                : asset('images/default-avatar.png');
                    @endphp

                    <img src="{{ $avatarUrl }}" alt="Profile Picture" class="avatar-image" id="profilePicturePreview"
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="first_name" value="{{ old('first_name', $admin->first_name) }}"
                                class="form-control track-change" data-original="{{ $admin->first_name }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Last Name</label>
                            <input type="text" name="last_name" value="{{ old('last_name', $admin->last_name) }}"
                                class="form-control track-change" data-original="{{ $admin->last_name }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Username</label>
                            <input type="text" name="username" value="{{ old('username', $admin->username) }}"
                                class="form-control track-change" data-original="{{ $admin->username }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Email</label>
                            <input type="email" name="email" value="{{ old('email', $admin->email) }}"
                                class="form-control track-change" data-original="{{ $admin->email }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Phone Number</label>
                            <input type="text" name="phone_number"
                                value="{{ old('phone_number', $admin->phone_number) }}" class="form-control track-change"
                                data-original="{{ $admin->phone_number }}">
                        </div>
                        <div class="col-md-6">

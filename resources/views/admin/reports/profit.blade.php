    ...
    <div class="container">
        <div class="report-card">
            ...
            <form id="report-form" action="#" method="GET" class="report-form">
                ...
                <div class="d-flex align-items-center me-4">
                    <label for="percentage-select" class="me-2 text-sm text-gray-700 font-semibold">Commission Percentage:</label>
...
<script>
    ...
    const allMonthlyData = {
        '2025-08': { profits: 1100, auctions: 7 },
        '2025-07': { profits: 850, auctions: 5 },
        '2025-06': { profits: 500, auctions: 3 },
        '2025-05': { profits: 700, auctions: 4 },
        '2025-04': { profits: 920, auctions: 6 }
    };
    ...
    function renderChartsAndTable(selectedMonthYear, commissionPercentage) {
        ...
        document.getElementById('monthly-average').textContent = `RM ${(monthData.profits / monthData.auctions).toFixed(2)}`;
        ...
        // Update chart data
        const labels = Object.keys(allMonthlyData).map(key => new Date(key + '-01').toLocaleString('en-US', { month: 'short' }));
        ...
        auctionChart = new Chart(auctionCtx, {
            type: 'doughnut',
            data: { labels: labels, datasets: [{ data: auctions, backgroundColor: ['#8b6239', '#a67c52', '#d4af37', '#b8941f', '#6d4c2f'], borderColor: '#ffffff', borderWidth: 3 }] },
        ...
        performanceChart = new Chart(performanceCtx, {
            type: 'bar',
            data: { labels: labels, datasets: [{ label: 'Average Profit per Auction', data: avgProfits, backgroundColor: ['rgba(139, 98, 57, 0.8)', 'rgba(166, 124, 82, 0.8)', 'rgba(212, 175, 55, 0.8)', 'rgba(184, 148, 31, 0.8)', 'rgba(109, 76, 47, 0.8)'], borderColor: ['#8b6239', '#a67c52', '#d4af37', '#b8941f', '#6d4c2f'], borderWidth: 2, borderRadius: 8 }] },
            options: { responsive: true, maintainAspectRatio: false }
        });
    }
    ...
    payNowBtn.addEventListener('click', function() {
        ...
        if (totalCommission > 0) {
            ...
            // In a real-world application, this would be where you'd redirect
            // to a payment gateway or submit a payment form.
            // Example: window.location.href = 'https://your-payment-gateway.com/pay?amount=' + totalCommission;
        } else {
            alert('No commission amount to pay.');
        }
    });

    // Load initial data at the beginning

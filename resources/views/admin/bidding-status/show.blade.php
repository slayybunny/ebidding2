            ...
            <div class="grid md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-100 shadow-md">
                    ...
                    <ul class="space-y-2 text-gray-600">
                        ...
                        <li><strong>Starting Price:</strong> RM {{ number_format($listing->starting_price, 2) }}</li>
                        <li><strong>Current Price:</strong> RM {{ number_format($currentPrice, 2) }}</li>
                        <li><strong>Status:</strong> {{ ucfirst($listing->status ?? 'N/A') }}</li>
                        <li><strong>Start Time:</strong> {{ $listing->created_at ?? 'N/A' }}</li>
                        <li><strong>End Time:</strong> {{ $listing->end_time ?? 'N/A' }}</li>
                    </ul>
                </div>
                <!-- Highest Bid Info Card -->
                <div class="bg-yellow-50 rounded-lg p-6 border border-yellow-200 shadow-md flex flex-col justify-center items-center text-center">
                    @if($highestBid)
                        <span class="text-5xl mb-2">🏆</span>
                        <p class="text-2xl font-bold text-yellow-800">Highest Bid</p>
                        <p class="text-4xl font-extrabold text-yellow-900 mt-2">RM {{ number_format($highestBid->bid_price, 2) }}</p>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse rounded-lg overflow-hidden">
                        <thead class="bg-gray-100 border-b-2 border-gray-200">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Bil</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Member</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Bid Price (RM)</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $bid->member->name ?? 'Unknown' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">RM {{ number_format($bid->bid_price, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ ucfirst($bid->status ?? 'pending') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $bid->created_at->format('d M Y, h:i A') }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="5" class="text-center py-8 text-gray-500 text-sm">No bids yet.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
